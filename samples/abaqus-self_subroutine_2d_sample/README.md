# Sample for a pseudo coupling depending on simulation results
...
> Take a look at [abaqus-self_2d_sample](samples/abaqus-self_2d_sample) before you start with this sample.
> Only difference: in this sample a user subroutine is used to extract simulation data and use it as an input for the pseudo coupling.
...
After each iteration in the Simulia Abaqus Standard simulation the actual pore pressure is exported into a csv file.
This file contains the deformed mesh coordinates and the corresponding pore pressure.
These information are imported into a new grid and will be used further.
In the upcoming four iteration steps the pore pressure results from the Simulia Abaqus simulation will be changed by a gaussian randomize function ```GaussRandomizeGrid.get_random_data_set()```.
...
![SimulationWorkflow](workflow.png "Flowchart showing the simulations workflow")
 ...
 11. Iterate through following steps: set step name; add step to Simulation Handler object; read pore pressure from previous step; randomize pore pressure values in grid and store for new step; create boundary conditions for input file; create Abaqus input file; create bash file; run simulation step;
